  ...
  useEffect(() => {
    if (preorder) {
      setFormData({
        ...
        delivery_status: preorder['Delivery Status'] || 'Pending'
      });
    }
  }, [preorder]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        alert(res.error || 'Failed to update preorder');
      }
    } catch (err: any) {
      alert(err?.response?.data?.error || 'Error updating preorder');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-slate-900 border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-amber-500/10 to-transparent">
          <h2 className="text-xl font-black text-white tracking-tight">Edit <span className="text-amber-400">Preorder</span></h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="label-xs text-slate-500 ml-1">Current Status</label>
                <select
...
export default EditPreorderModal;
